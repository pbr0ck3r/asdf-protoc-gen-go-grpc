...
# NOTE: You might want to remove this if protoc-gen-go-grpc is not hosted on GitHub releases.
...
list_github_tags() {
  git ls-remote --tags --refs "$GH_REPO" |
    grep -o 'refs/tags/cmd/protoc-gen-go-grpc/.*' | cut -d/ -f5- |
    sed 's/^v//' # NOTE: You might want to adapt this sed to remove non-version strings from tags
}

list_all_versions() {
  # TODO: Adapt this. By default we simply list the tag names from GitHub releases.
  # Change this function if protoc-gen-go-grpc has other means of determining installable versions.
  list_github_tags
}
